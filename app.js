...
let stairGroup = null;
let stairMeshes = {}; // Map stair ID to mesh group for selection/dragging

/**
 * Initialize the stair group (call during initScene)
 */
...
// ===========================================
// STAIR POSITION CALCULATIONS
// ===========================================

/**
 * Get the world position for a stair based on edge and position
 */
function getStairWorldPosition(stairConfig, dimensions) {
    const edge = stairConfig.edge;
    const position = stairConfig.position; // 0-1 along edge
    const width = stairConfig.width;
...
function getStairRotation(edge) {
    switch (edge) {
        case 'front': return 0;                    // Facing -Z
        case 'back': return Math.PI;               // Facing +Z
        case 'left': return Math.PI / 2;           // Facing -X
        case 'right': return -Math.PI / 2;         // Facing +X
        default: return 0;
    }
}

/**
 * Get the direction vector for stair descent
 */
function getStairDirection(edge) {
    switch (edge) {
...
/**
 * Create material for stair stringers (neutral lumber)
 */
...
/**
 * Create material for stair treads (uses deck color)
 */
function createTreadMaterial(colorConfig, boardLength, uniqueId) {
    return createBoardMaterial(colorConfig, boardLength, false, `stair_tread_${uniqueId}`);
}

/**
 * Create material for stair risers (uses deck color)
 */
function createRiserMaterial(colorConfig, boardLength, uniqueId) {
    return createBoardMaterial(colorConfig, boardLength, false, `stair_riser_${uniqueId}`);
}

/**
 * Create material for handrails (white, matching deck railings)
 */
...
// ===========================================
// GEOMETRY CREATION - STRAIGHT STAIRS
// ===========================================

/**
 * Create all stairs
 * Call this from buildDeck() after createDetailedRailings()
 */
function createStairs() {
    ...
    for (const stair of state.stairs) {
        ...
        if (stair.shape === 'l-shaped') {
            createLShapedStair(stair, dims, stairMeshGroup, colorConfig);
        } else {
            createStraightStair(stair, dims, stairMeshGroup, colorConfig);
        }

        // Position and rotate the stair group
        const worldPos = getStairWorldPosition(stair, dims);
        const rotation = getStairRotation(stair.edge);
 ...
 */
function createStraightStair(stairConfig, dims, parentGroup, colorConfig) {
    const direction = getStairDirection(stairConfig.edge);

    // Create stringers
    createStairStringers(stairConfig, dims, parentGroup);

    // Create treads and risers
    createStairTreadsAndRisers(stairConfig, dims, parentGroup, colorConfig);

    // Create handrails if enabled
    if (stairConfig.includeHandrails) {
        createStairHandrails(stairConfig, dims, parentGroup);
    }
}
...
function createStairStringers(stairConfig, dims, parentGroup) {
    ...
    const riseTotal = dims.deckHeightInches / 12;  // feet
    ...
    const stringerThickness = CONFIG.stairs.stringerThickness / 12;  // feet
    const stringerWidth = CONFIG.stairs.stringerWidth / 12;          // feet
    const stringerInset = CONFIG.stairs.stringerInset / 12;          // feet
    ...
    let stringerCount = 2;
    if (dims.stairWidthFeet > 3) stringerCount = 3;
    if (dims.stairWidthFeet > 6) stringerCount = 4;
    ...
    // Outer stringers
    positions.push(-halfWidth + stringerInset);
    positions.push(halfWidth - stringerInset);

    // Center stringers if needed
    if (stringerCount >= 3) {
        positions.push(0);
    }
    if (stringerCount >= 4) {
        positions.splice(1, 0, -halfWidth / 2);
        positions.splice(3, 0, halfWidth / 2);
    }

    positions.forEach((xPos, index) => {
        const stringer = new THREE.Mesh(stringerGeometry.clone(), stringerMaterial);

        // Position: center of the stringer run, angled
        const centerX = 0;
        const centerY = state.deckHeight / 2;
...
function createStairTreadsAndRisers(stairConfig, dims, parentGroup, colorConfig) {
    ...
    const boardLength = window.stairFunctions ? 
        selectOptimalBoardLength(stairWidthFt) : 
        (stairWidthFt <= 4 ? 12 : (stairWidthFt <= 8 ? 12 : 16));
    ...
    const finalRiserGeometry = new THREE.BoxGeometry(
        stairWidthFt,
        risePerStepFt,
        boardThicknessFt
    );
    ...
    finalRiser.position.set(0, finalRiserY, finalRiserZ);
...
/**
 * Create stair handrails (matching deck railing style)
 */
function createStairHandrails(stairConfig, dims, parentGroup) {
    const handrailMaterial = createHandrailMaterial();

    const postHeight = 3;  // feet (matching deck railings)
    const postSize = 0.29; // feet
    const railHeight = 0.29;
    const railThickness = 0.125;
    const balusterSize = 0.125;
    const balusterSpacing = 0.33;
    ...
    [-1, 1].forEach(side => {
        ...
        // Intermediate posts along the stair
        const postSpacing = CONFIG.stairs.handrailPostSpacing;
        const numIntermediatePosts = Math.floor(runTotal / postSpacing) - 1;

        for (let i = 1; i <= numIntermediatePosts; i++) {
            const t = i / (numIntermediatePosts + 1);
            const postZ = -t * runTotal;
            const postY = state.deckHeight - (t * riseTotal) + postHeight / 2;

            const intermediatePost = new THREE.Mesh(postGeometry, handrailMaterial);
            intermediatePost.position.set(xPos, postY, postZ);
            intermediatePost.castShadow = true;
            sideGroup.add(intermediatePost);
        }
...
// ===========================================
// GEOMETRY CREATION - L-SHAPED STAIRS
// ===========================================

/**
 * Create an L-shaped stair with landing
 */
function createLShapedStair(stairConfig, dims, parentGroup, colorConfig) {
    if (!dims.lShapedData) return;

    const lData = dims.lShapedData;
    const direction = getStairDirection(stairConfig.edge);
    const turnDir = stairConfig.turnDirection === 'left' ? -1 : 1;

    // First flight (from deck to landing)
    const firstFlightGroup = new THREE.Group();
    createLShapedFirstFlight(stairConfig, dims, firstFlightGroup, colorConfig);
    parentGroup.add(firstFlightGroup);

    // Landing platform
    const landingGroup = new THREE.Group();
    createStairLanding(stairConfig, dims, landingGroup, colorConfig);
    parentGroup.add(landingGroup);

    // Second flight (from landing to ground)
    const secondFlightGroup = new THREE.Group();
    createLShapedSecondFlight(stairConfig, dims, secondFlightGroup, colorConfig, turnDir);
    parentGroup.add(secondFlightGroup);

    // Handrails for L-shaped stairs
    if (stairConfig.includeHandrails) {
        createLShapedHandrails(stairConfig, dims, parentGroup, turnDir);
    }
}

/**
 * Create first flight of L-shaped stairs (deck to landing)
 */
function createLShapedFirstFlight(stairConfig, dims, parentGroup, colorConfig) {
    const lData = dims.lShapedData;
    const boardWidthFt = CONFIG.boards.width / 12;
    const boardThicknessFt = CONFIG.boards.thickness / 12;
    const gapFt = CONFIG.boards.gap / 12;
    ...
    createFirstFlightStringers(stairConfig, dims, parentGroup);

    // Treads and risers
    for (let step = 0; step < lData.treadsBeforeLanding; step++) {
        const stepY = state.deckHeight - ((step + 1) * risePerStepFt);
        const stepZ = -((step + 1) * treadDepthFt) + (treadDepthFt / 2);
        ...
        for (let board = 0; board < dims.boardsPerTread; board++) {
            ...
            const treadMaterial = createTreadMaterial(colorConfig, boardLength, `f1_${step}_${board}`);
        ...
        const riserMaterial = createRiserMaterial(colorConfig, boardLength, `f1_riser_${step}`);
...
/**
 * Create stringers for first flight
 */
function createFirstFlightStringers(stairConfig, dims, parentGroup) {
    const lData = dims.lShapedData;
    const stringerMaterial = createStringerMaterial();

    const riseTotal = lData.heightAtLanding / 12;
    const runTotal = lData.run1Feet;
    const stringerLength = Math.sqrt(riseTotal * riseTotal + runTotal * runTotal);
    const angle = Math.atan2(riseTotal, runTotal);
    ...
    const stringerGeometry = new THREE.BoxGeometry(stringerLength, stringerWidth, stringerThickness);

    const halfWidth = dims.stairWidthFeet / 2;
    const positions = [-halfWidth + stringerInset, halfWidth - stringerInset];
    if (dims.stairWidthFeet > 3) positions.push(0);

    positions.forEach(xPos => {
        const stringer = new THREE.Mesh(stringerGeometry.clone(), stringerMaterial);
        const centerY = state.deckHeight - (riseTotal / 2);
        const centerZ = -runTotal / 2;
 ...
 */
function createStairLanding(stairConfig, dims, parentGroup, colorConfig) {
    const lData = dims.lShapedData;
    const boardWidthFt = CONFIG.boards.width / 12;
    const boardThicknessFt = CONFIG.boards.thickness / 12;
    const gapFt = CONFIG.boards.gap / 12;

    const landingY = state.deckHeight - (lData.heightAtLanding / 12);
    const landingZ = -(lData.run1Feet + lData.landingDepthFeet / 2);
    const landingWidth = dims.stairWidthFeet;
    const landingDepth = lData.landingDepthFeet;

    const boardLength = selectOptimalBoardLength(landingWidth);
    const numBoards = Math.ceil(landingDepth / (boardWidthFt + gapFt));

    // Landing deck boards
    for (let i = 0; i < numBoards; i++) {
        const boardZ = landingZ - (landingDepth / 2) + (i * (boardWidthFt + gapFt)) + (boardWidthFt / 2);

        const boardMaterial = createTreadMaterial(colorConfig, boardLength, `landing_${i}`);
        const boardGeometry = new THREE.BoxGeometry(landingWidth, boardThicknessFt, boardWidthFt);

        const board = new THREE.Mesh(boardGeometry, boardMaterial);
        board.position.set(0, landingY + boardThicknessFt / 2, boardZ);
        board.castShadow = true;
        board.receiveShadow = true;
 ...
 */
function createLShapedSecondFlight(stairConfig, dims, parentGroup, colorConfig, turnDir) {
    const lData = dims.lShapedData;
    const boardWidthFt = CONFIG.boards.width / 12;
    const boardThicknessFt = CONFIG.boards.thickness / 12;
    const gapFt = CONFIG.boards.gap / 12;
    ...
    const landingY = state.deckHeight - (lData.heightAtLanding / 12);
    const landingEndZ = -(lData.run1Feet + lData.landingDepthFeet);

    // Second flight runs perpendicular (turned 90 degrees)
    const flightGroup = new THREE.Group();

    // Create stringers for second flight
    createSecondFlightStringers(stairConfig, dims, flightGroup, turnDir);

    // Create treads and risers for second flight
    for (let step = 0; step < lData.treadsAfterLanding; step++) {
        const stepY = landingY - ((step + 1) * risePerStepFt);
        const stepLocalZ = -((step + 1) * treadDepthFt) + (treadDepthFt / 2);

        // Tread boards
        for (let board = 0; board < dims.boardsPerTread; board++) {
            const boardLocalZ = stepLocalZ - (treadDepthFt / 2) + (board * (boardWidthFt + gapFt)) + (boardWidthFt / 2);

            const treadMaterial = createTreadMaterial(colorConfig, boardLength, `f2_${step}_${board}`);
            const treadGeometry = new THREE.BoxGeometry(stairWidthFt, boardThicknessFt, boardWidthFt);

            const tread = new THREE.Mesh(treadGeometry, treadMaterial);
            tread.position.set(0, stepY + boardThicknessFt / 2, boardLocalZ);
            tread.castShadow = true;
            tread.receiveShadow = true;

            flightGroup.add(tread);
        }

        // Riser
        const riserMaterial = createRiserMaterial(colorConfig, boardLength, `f2_riser_${step}`);
        const riserGeometry = new THREE.BoxGeometry(stairWidthFt, risePerStepFt, boardThicknessFt);

        const riser = new THREE.Mesh(riserGeometry, riserMaterial);
        riser.position.set(0, stepY - (risePerStepFt / 2) + boardThicknessFt, stepLocalZ - (treadDepthFt / 2));
        riser.castShadow = true;
        riser.receiveShadow = true;

        flightGroup.add(riser);
    }
 ...
 * Create stringers for second flight
 */
function createSecondFlightStringers(stairConfig, dims, parentGroup, turnDir) {
    const lData = dims.lShapedData;
    const stringerMaterial = createStringerMaterial();

    const landingY = state.deckHeight - (lData.heightAtLanding / 12);
    const riseTotal = landingY;
    const runTotal = lData.run2Feet;
    const stringerLength = Math.sqrt(riseTotal * riseTotal + runTotal * runTotal);
    const angle = Math.atan2(riseTotal, runTotal);
...
/**
 * Create handrails for L-shaped stairs
 */
function createLShapedHandrails(stairConfig, dims, parentGroup, turnDir) {
    // Simplified: use same handrail creation for each flight
    // Full implementation would connect the handrails at the landing

    const handrailMaterial = createHandrailMaterial();
    const postHeight = 3;
    const postSize = 0.29;

    // Corner posts at landing
    const lData = dims.lShapedData;
    const landingY = state.deckHeight - (lData.heightAtLanding / 12);
    const landingZ = -(lData.run1Feet + lData.landingDepthFeet / 2);
    const halfWidth = dims.stairWidthFeet / 2;
    ...
    // Landing corner posts
    const cornerPositions = [
        { x: -halfWidth, z: landingZ - lData.landingDepthFeet / 2 },
        { x: halfWidth, z: landingZ - lData.landingDepthFeet / 2 },
        { x: turnDir * (halfWidth + lData.run2Feet), z: landingZ - lData.landingDepthFeet / 2 - halfWidth * turnDir }
    ];

    cornerPositions.forEach(pos => {
        const post = new THREE.Mesh(postGeometry, handrailMaterial);
        post.position.set(pos.x, landingY + postHeight / 2, pos.z);
        post.castShadow = true;
...
console.log('Agent 2: Stair 3D Geometry loaded');
